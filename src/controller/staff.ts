...
//   try {
//     await Promise.all([
//       db.delete(userOutletRoles).where(
//         and(eq(userOutletRoles.userId, userId), eq(userOutletRoles.outletId, outletId))
//       ),
//       db.delete(userOutlets).where(
//         and(eq(userOutlets.userId, userId), eq(userOutlets.outletId, outletId))
//       ),
//     ]);
...
//     return { success: false, error: "Failed to remove staff member", status: 500 };
//   }
// }
...
export async function assignExistingStaff(
  ...
  if (!targetUser) {
    return {
      success: false,
      error:
        "No user found with this email. Use 'create' to add a new account.",
      status: 404,
    };
  }
  ...
  const alreadyAssigned = await db.query.userOutlets.findFirst({
    where: (uo, { eq, and }) =>
      and(eq(uo.userId, targetUser.id), eq(uo.outletId, outletId)),
  });
  if (alreadyAssigned) {
    return {
      success: false,
      error: "This user already has access to this outlet",
      status: 409,
    };
  }
  ...
  try {
    ...
    await db
      .insert(userOutletRoles)
      .values({ userId: targetUser.id, outletId, roleId });
