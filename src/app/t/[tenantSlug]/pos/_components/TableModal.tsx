...
export default function TableModal({ table, tenantSlug, role, onClose, onStatusChange, onKotStatusChange }: TableModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
      <div className={`w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden rounded-2xl border transition-all duration-200 ${
        isDark ? "bg-[#0c0c0d] border-neutral-800 text-white" : "bg-white border-slate-200 text-slate-800"
      }`}>

        {/* Header */}
        <div className={`flex flex-col sm:flex-row sm:items-center justify-between px-6 pt-5 pb-4 gap-4 shrink-0 border-b ${
          isDark ? "border-neutral-900/50" : "border-slate-100"
        }`}>
          ...
          <div className={`flex items-center gap-1 p-1 rounded-xl border ${
            isDark ? "bg-[#141416] border-neutral-800" : "bg-slate-100 border-slate-200"
          }`}>
          ...
          <button
            onClick={onClose}
            className={`transition-colors p-2 rounded-lg self-start sm:self-center ${
        ...
        {/* Tab Bar */}
        <div className={`flex items-center gap-1 px-6 pt-2 pb-0 border-b shrink-0 ${
          isDark ? "border-neutral-800" : "border-slate-200 bg-slate-50/50"
        }`}>
          {TABS.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {tab === 'Order List' && orders.length > 0 && (
                  <span className={`ml-1 inline-flex items-center justify-center w-5 h-5 text-[10px] font-bold rounded-full ${
                    isDark ? 'bg-[#e5b83b] text-[#0c0c0d]' : 'bg-emerald-600 text-white'
                  }`}>
                    {orders.length}
                  </span>
                )}
        ...
        <div className="flex-1 overflow-hidden">
          ...
          {activeTab === 'Order List' && (
            <div className="h-full flex flex-col justify-between overflow-hidden">
                  ...
                  orders.map((order) => {
                    ...
                    return (
                      <div key={order.id} className={`border rounded-xl p-4 flex flex-col gap-3 ${
                        isDark ? "bg-[#141416] border-neutral-800" : "bg-slate-50 border-slate-200"
                      }`}>
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${
                                isDelivered
                                  ? isDark ? 'bg-neutral-900 border-neutral-800 text-neutral-500' : 'bg-slate-200 border-slate-300 text-slate-500'
                                  : isDark ? 'bg-[#e5b83b]/10 border-[#e5b83b]/20 text-[#e5b83b]' : 'bg-emerald-50 border-emerald-200 text-emerald-700'
                              }`}>
                              ...
                              <button
                                ...
                                className={`ml-1 p-1 rounded-md transition-all duration-150 disabled:opacity-40 ${
                                  isDark ? 'text-neutral-500 hover:text-red-400 hover:bg-red-500/10' : 'text-slate-400 hover:text-red-600 hover:bg-red-50'
                                }`}
                                title="Cancel Order"
                              >
                                {isDeleting ? (
                        ...
                        <div className={`space-y-2 border-t pt-2 ${isDark ? "border-neutral-900/50" : "border-slate-200"}`}>
                          {order.items.map((item, idx) => (
                            <div key={idx} className="flex justify-between text-xs">
                              <span className={isDark ? "text-neutral-400" : "text-slate-500"}>{item.quantity} × {item.name}</span>
                              <span className={isDark ? "text-neutral-200" : "text-slate-700 font-medium"}>Rs.{item.subtotal.toFixed(2)}</span>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className={`pt-2 border-t flex items-center justify-between gap-2 ${isDark ? "border-neutral-900/40" : "border-slate-200"}`}>
                          <div>
                            {!isDelivered && (
                              <div className={`flex items-center gap-1.5 text-[10px] font-bold tracking-wide uppercase px-2.5 py-1.5 rounded-lg border ${
                                isDark ? "border-neutral-900 bg-neutral-950/20 text-neutral-500" : "border-slate-200 bg-slate-100 text-slate-500"
                              }`}>
                          ...
                          {showDeliveryButton && (
                            <button
                              ...
                              className={`text-[10px] font-bold tracking-wide uppercase px-2.5 py-1.5 rounded-lg transition-colors border ${
                                isDelivered
                                  ? isDark ? 'border-neutral-800 text-neutral-400 hover:text-white hover:bg-neutral-900' : 'border-slate-300 text-slate-500 hover:text-slate-700 hover:bg-slate-100'
                                  : isDark ? 'bg-[#e5b83b] border-transparent text-[#0c0c0d] hover:bg-[#f5c847]' : 'bg-emerald-600 border-transparent text-white hover:bg-emerald-700'
                              }`}
                            >
              ...
              {orders.length > 0 && (
                <div className={`border-t p-5 flex items-center justify-between gap-6 shrink-0 ${
      ...
      {deleteOrderConfirmId && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[70] animate-in fade-in duration-200">
          <div className={`w-full max-w-sm rounded-xl border p-6 flex flex-col gap-4 transition-all duration-200 ${
            ...
            <div className="flex gap-3 mt-2">
              <button
                ...
                className={`flex-1 font-bold py-2.5 rounded-xl transition-all duration-150 text-xs border ${
                  isDark
              ...
              <button
                ...
                disabled={isDeletingId !== null}
                className={`flex-1 font-bold py-2.5 rounded-xl transition-all duration-150 text-xs flex items-center justify-center gap-1.5 ${
                  isDark
