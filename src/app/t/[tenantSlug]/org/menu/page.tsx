...
import {
  ...
  UtensilsCrossed, Loader2, Settings2, ImagePlus, Store, ChevronDown, Ruler
} from "lucide-react";
...
export default function MenuManagement() {
  ...
  async function syncVariants(
    ...
    for (const size of updated) {
      if (size.id) {
        ...
        await api.post(`/product/${productId}/variants?outletId=${outletId}`, {
          label: size.label,
          price: size.price.toFixed(2),
        });
      }
  ...
  const handleSaveItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (savedId) {
        if (draft.hasSizes) {
          await syncVariants(savedId, activeOutletId, originalSizes, validSizes);
