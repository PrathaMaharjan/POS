...
import { getDateRange, getPaymentGatewayShare } from "@/controller/org/controller";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const data = await getPaymentGatewayShare(
    ...
    return NextResponse.json({ error: "Failed to fetch payment share" }, { status: 500 });
  }
}